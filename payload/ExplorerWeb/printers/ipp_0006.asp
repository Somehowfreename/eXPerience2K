...
<%
    ...
    if Len(strPrinter) > MAX_ENCODED_PRINTER_NAME_LENGTH Then
        Call ErrorHandler(L_Invalid_Parameter)
    Else
...
End Function
    ...
    strText = RepString2(L_Title_Text, strPrinter, session(LOCAL_SERVER))
...
Function GenPropertyTable (objQueue, objPrinter)
    ...
    strHTML = strHTML & strBgnCol1 & L_PrinterModel_Text & strBgnCol2 & objQueue.model & strEndCol
    ...
    strUrl = strUrl & session(LOCAL_SERVER) & "/printers/" & GetFriendlyName (objQueue.Printerpath, strComputer) & "/.printer"
